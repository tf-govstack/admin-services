...
public class Utilities {
	...
	private final Logger logger = HotlistLogger.getLogger(Utilities.class);
	/** The reg proc logger. */
    ...
    public JSONObject retrieveIdrepoJson(String uin) throws ApisResourceAccessException, HotlistAppException, IOException {
		...
		if (uin != null) {
			...
			idResponseDto = (IdResponseDTO1) hotlistServiceRestClient.getApi(ApiName.IDREPOGETIDBYUIN, pathSegments, "", "",
			...
			if (!idResponseDto.getErrors().isEmpty()) {
				...
				throw new HotlistAppException(ResidentErrorCode.RESIDENT_SYS_EXCEPTION.getErrorCode(), error.get(0).getMessage());
			}
			...
			try {
				...
				throw new HotlistAppException(ResidentErrorCode.RESIDENT_SYS_EXCEPTION.getErrorCode(), "Error while parsing string to JSONObject",e);
			}
...
/*
	public String getUinByVid(String vid) throws ApisResourceAccessException, VidCreationException, IOException {
		...
		response = (VidResponseDTO1) hotlistServiceRestClient.getApi(ApiName.GETUINBYVID, pathSegments, "", "",
	...
	public boolean linkRegIdWrtUin(String registrationID, String uin) throws ApisResourceAccessException, IOException {
		...
		if (uin != null) {
			...
			idResponse = (IdResponseDTO) hotlistServiceRestClient.patchApi(env.getProperty(ApiName.IDREPOSITORY.name()), MediaType.APPLICATION_JSON, idRequestDTO,
					IdResponseDTO.class);
...
/*
	...
	*/
    ...
    public String getPhoneAttribute() throws HotlistAppException {
    	return getIdMappingAttributeForKey(MappingJsonConstants.PHONE);
    }
    
    public String getEmailAttribute() throws HotlistAppException {
    	return getIdMappingAttributeForKey(MappingJsonConstants.EMAIL);
    }

	private String getIdMappingAttributeForKey(String attributeKey) throws HotlistAppException {
		try {
			JSONObject regProcessorIdentityJson = getRegistrationProcessorMappingJson();
